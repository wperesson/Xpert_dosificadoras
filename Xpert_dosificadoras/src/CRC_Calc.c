/*
 * CRC_Calc.c
 *
 *  Created on: 28/08/2013
 *      Author: admin
 */

/*
 ** Tables for generating CRC16 checksum
 */

//#include "Definiciones.h";
#include "lpc_types.h"

const uint8_t u8Crc16Hi[] = {  };
const uint8_t u8Crc16Lo[] = {  };

/*
 **---------------------------------------------------------------------------
 **
 ** Function:
 ** Crc16()
 **
 ** Abstract:
 ** Calculate a CRC16 checksum.
 **
 ** Comments:
 ** None
 **
 ** Parameters:
 ** pbBufferStart Pointer to start of buffer
 ** u16Length Number of character is buffer over which to
 ** calculate CRC16
 **
 ** Returns:
 ** CRC16 value
 **
 **---------------------------------------------------------------------------
 */
uint16_t Crc16(uint8_t *pu8BufferStart, uint16_t u16Length) {
	uint8_t u8Index;
	uint8_t u8CrcLo;
	uint8_t u8CrcHi;
	u8CrcLo = 0xFF; // Init CRC to 0xFFFF
	u8CrcHi = 0xFF;
// CRC calculation
	while (u16Length > 0) {
		u8Index = u8CrcLo ^ *pu8BufferStart++;
		u8CrcLo = u8CrcHi ^ u8Crc16Hi[u8Index];
		u8CrcHi = u8Crc16Lo[u8Index];
		u16Length--;
	}
	return ((u8CrcHi << 8) | u8CrcLo);
}

/*
 **---------------------------------------------------------------------------
 **
 ** Function:
 ** MBCrc16()
 **
 ** Abstract:
 ** Calculate a CRC16 checksum.
 **
 ** Comments:
 ** None
 **
 ** Parameters:
 ** pbBufferStart Pointer to start of buffer
 ** u16Length Number of character is buffer over which to
 ** calculate CRC16
 **
 ** Returns:
 ** CRC16 value
 **
 **---------------------------------------------------------------------------
 */
void MBCrc16(uint8_t *pu8BufferStart, uint16_t u16Length) {
	uint8_t u8Index;
	uint8_t u8CrcLo;
	uint8_t u8CrcHi;
	u8CrcLo = 0xFF; // Init CRC to 0xFFFF
	u8CrcHi = 0xFF;
// CRC calculation
	while (u16Length > 0) {
		u8Index = u8CrcLo ^ *pu8BufferStart++;
		u8CrcLo = u8CrcHi ^ u8Crc16Hi[u8Index];
		u8CrcHi = u8Crc16Lo[u8Index];
		u16Length--;
	}
	*pu8BufferStart++ = u8CrcLo;
	*pu8BufferStart = u8CrcHi;
}

unsigned int crc_16(unsigned char *buffer, unsigned int length) {
	unsigned int i, j, temp_bit, temp_int, crc;
	crc = 0xFFFF;
	for (i = 0; i < length; i++) {
		temp_int = (unsigned char) *buffer++;
		crc ^= temp_int;
		for (j = 0; j < 8; j++) {
			temp_bit = crc & 0x0001;
			crc >>= 1;
			if (temp_bit != 0)
				crc ^= 0xA001;
		}
	}
}
