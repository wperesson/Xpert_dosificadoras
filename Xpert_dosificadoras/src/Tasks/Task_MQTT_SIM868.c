...
//	sc_gprs_on,
//	sc_gprs_on_init,
//	sc_gprs_on_r2_op_not_conn,
//	sc_gprs_on_r2_op_conn,
//} mqtt_states;
...
struct on_state;
typedef void on_state_fn(struct on_state*);
struct on_state {
	on_state_fn *next;
	uint32_t event;
	uint32_t timeout;
...
main_state_fn sc_init, sc_gprs_off, sc_gprs_on;

on_state_fn sc_on_init, sc_on_op_not_conn, sc_gprs_on_op_conn;

void sc_init(struct main_state *state) {
	state->next = sc_gprs_off;
	state->timeout = 10;
}

void sc_gprs_off(struct main_state *state) {

	if (state->event == mqtt_ev_gprs_on)
		state->next = sc_gprs_on;
}

void sc_gprs_on(struct main_state *state) {
...
//	mqtt_prev_st = mqtt_ev_connect_to_broker;
	next_event(mqtt_ev_gprs_on); //Enciendo el gprs
	next_event(mqtt_ev_req_keys_from_broker);

	/*Inicio la máquina de estados*/
//	state = sc_init;
	/* init FSM */
	struct main_state main_state = { sc_init, "", false, false, false, false };
...
//		if (xQueueReceive(queMQTT_ev, &mqtt_ev, main_state.timeout) == pdPASS) {
//		} else {
//			mqtt_ev = mqtt_ev_NO_EV;
//		}
//
//		/* update FSM */
//		main_state.event = mqtt_ev;
